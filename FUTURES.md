...
2. Manually prefund that wallet with USD only. No automatic conversion, transfer, withdrawal, top-up or balance import is implemented. At first arming, its USD balance must equal the explicitly configured **Futures live collateral · USD**, and the full amount must be available. The allocation is bound to the authenticated account ID and cannot be silently resized or adopted by another wallet after changing credentials. Do not deposit, withdraw, convert or trade externally while Kairos manages this wallet.
