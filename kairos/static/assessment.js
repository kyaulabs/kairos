...
class AssessmentView {
  ...
  render(decision, state, events = [], definition = {}) {
    ...
    if (definition.scheduled) {
    ...
    } else if (rules && Array.isArray(input.series) && input.series.length > 1) {
      ...
      if (series.length > 1 && levels.every(row => row.value > 0)) {
      ...
      svg.attr('aria-label', rows ? `Model directional bias ${(100*(rows[0].value-rows[2].value)).toFixed(1)} percentage points; ${rows.map(r=>`${r.action} ${(r.value*100).toFixed(1)} percent`).join(', ')}. Not profitability.` : rules ? 'No current Bollinger window; no model probabilities.' : 'No current model assessment.');
      text(180,18,rows ? 'BUY − SELL · MODEL BIAS' : rules ? 'RANGE · AWAITING DATA' : 'MODEL · AWAITING DATA','instrument-caption','middle');
      const arc=d3.arc().innerRadius(73).outerRadius(84);
      svg.append('g').attr('transform','translate(180,120)').selectAll('path').data(d3.range(30)).join('path').attr('d',i=>arc({startAngle:-Math.PI/2+i*Math.PI/30+.008,endAngle:-Math.PI/2+(i+1)*Math.PI/30-.008})).attr('fill',i=>i<14?'var(--danger)':i>15?'var(--success)':'var(--muted)').attr('opacity',rows ? .35 : .12);
      if(rows) {
        const bias=rows[0].value-rows[2].value, angle=bias*Math.PI/2;
        svg.append('line').attr('x1',180+Math.sin(angle)*65).attr('y1',120-Math.cos(angle)*65).attr('x2',180+Math.sin(angle)*89).attr('y2',120-Math.cos(angle)*89).attr('stroke','var(--text)').attr('stroke-width',3).attr('stroke-linecap','round');
        text(180,110,`${bias>0?'+':''}${(bias*100).toFixed(1)}`,'instrument-score','middle');
      ...
      } else text(180,114,'—','instrument-score','middle');
      text(85,143,'SELL','instrument-caption','middle');text(275,143,'BUY','instrument-caption','middle');
    }
    if (!rules && decision) {
    ...
    const metrics=definition.scheduled ? [['ORDERS',state.program?.orders || 0],['MISSED SLOTS',state.program?.skipped_slots || 0],['TURNOVER · USD',state.program?.spent_including_fees || 0]] : rules ? [['WINDOW Z', input.z_score],['TREND ER',input.efficiency],[input.position ? 'ENTRY NET · BPS' : 'NET ROOM · BPS',input.net_room_bps]] : [['TREND',input.trend],['SPREAD · BPS',input.spread_bps],['INVENTORY',input.inventory]];
    ...
    if(rules) root.append('p').attr('class','assessment-protection muted').text(position ? `Protection plan · stop ${Number(position.stop).toLocaleString(undefined,{maximumFractionDigits:8})} · target ${Number(position.target).toLocaleString(undefined,{maximumFractionDigits:8})} · deadline ${new Date(position.deadline*1000).toLocaleTimeString()}. Checks run only while started.` : 'Flat · one position maximum · fixed midpoint target · price stop · holding deadline');
  }
}
